...
TOOLS = [
    ...
    },
    {
        "name": "dops",
        "executable": "dops",
        "description": "Better Docker container listing",
...
def check_tools():
    ...
    print(f"{Colors.BOLD}--- Tools ---{Colors.END}", end="")
    uninstalled_tools = []
    ...
    for i, tool in enumerate(TOOLS):
        ...
        if is_installed:
            ...
            status = f"{Colors.RED}✖ Not found{Colors.END}"
            uninstalled_tools.append(tool)

        tool_string = (
            f"\n{i}. {Colors.BOLD}{Colors.BLUE}{tool['name']}{Colors.END} ({status})"
        )

        print(f"{tool_string}: {tool['description']}", end="")
...
def install_tool(tool):
    ...
    if tool["install_cmd"] is None:
        print(
            f"{Colors.RED}Unsupported system: No package manager found for this tool.{Colors.END}"
        )
        return
    ...
    try:
        ...
        print(
            f"{Colors.RED}Installation failed for {tool['name']}. Error: {e}{Colors.END}"
        )
    except FileNotFoundError:
        cmd_name = (
            tool["install_cmd"][0]
            if isinstance(tool["install_cmd"], list)
            else tool["install_cmd"].split()[0]
        )
...
def installation_menu(uninstalled_tools):
    ...
    while True:
        ...
        for i, tool in enumerate(uninstalled_tools):
            print(f"  {Colors.BOLD}{tool['id']}{Colors.END}) Install {tool['name']}")
        ...
        elif choice.isdigit() and int(choice) in [t["id"] for t in uninstalled_tools]:
            tool_to_install = [
                tool for tool in uninstalled_tools if tool["id"] == int(choice)
            ]
            if len(tool_to_install) > 1:
                raise RuntimeError("Multiple tools identified for id {choice}")

            install_tool(tool_to_install[0])
            break
        else:
